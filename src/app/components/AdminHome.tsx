import { User, Bell, Users, Activity, TrendingUp, FileText, Settings, BarChart3, Calendar, Stethoscope } from 'lucide-react';
            ...
            <div className="p-6 rounded-3xl bg-white/50 backdrop-blur-sm border border-white/60 shadow-xl">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <AreaChart data={loginData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: 'rgba(255,255,255,0.95)', border: '1px solid #e0e0e0', borderRadius: '12px' }} />
                  <Area type="monotone" dataKey="clientes" stackId="1" stroke="#001389" fill="#001389" fillOpacity={0.6} />
                  <Area type="monotone" dataKey="medicos" stackId="1" stroke="#D52A1D" fill="#D52A1D" fillOpacity={0.6} />
                  <Area type="monotone" dataKey="colaboradors" stackId="1" stroke="#4CAF50" fill="#4CAF50" fillOpacity={0.6} />
                </AreaChart>
              </ResponsiveContainer>
              <div className="flex items-center justify-center gap-6 mt-4">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#001389]" /><span className="text-xs text-gray-600">Clientes</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#D52A1D]" /><span className="text-xs text-gray-600">Médicos</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#4CAF50]" /><span className="text-xs text-gray-600">Colaboradores</span></div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl bg-white/50 backdrop-blur-sm border border-white/60 shadow-xl">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={doctoresRegistradosData}>
                  ...
                  <YAxis stroke="#666" style={{ fontSize: '12px' }} />
                  <Tooltip contentStyle={{ backgroundColor: 'rgba(255,255,255,0.95)', border: '1px solid #e0e0e0', borderRadius: '12px' }} />
                  <Line type="monotone" dataKey="doctores" stroke="#D52A1D" strokeWidth={3} dot={{ fill: '#D52A1D', r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
              <div className="flex items-center justify-between mt-4 px-2">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#D52A1D]" /><span className="text-xs text-gray-600">Check-ins por hora</span></div>
            ...
            <div className="p-6 rounded-3xl bg-white/50 backdrop-blur-sm border border-white/60 shadow-xl">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={consultasData}>
                  ...
                  <YAxis stroke="#666" style={{ fontSize: '12px' }} />
                  <Tooltip contentStyle={{ backgroundColor: 'rgba(255,255,255,0.95)', border: '1px solid #e0e0e0', borderRadius: '12px' }} />
                  <Bar dataKey="consultas" fill="#001389" radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
